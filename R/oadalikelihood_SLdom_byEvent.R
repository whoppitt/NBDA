...
oadaLikelihood_SLdom_byEvent <- function(parVect, nbdadata){
  ...
  if(nbdadata@multi_ilv[1]=="ILVabsent") noILVmulti<-0
	...
	solverTotalRate<-asocialRate[nbdadata@status==1]*(tapply(socialRate, INDEX=event.id_factor, FUN=sum)==0)+socialRate[nbdadata@status==1]*(tapply(socialRate, INDEX=event.id_factor, FUN=sum)>0)


	#Take logs
	lComp1 <- log(solverTotalRate)# group by skilled

	lComp2.1 <-tapply(asocialRate, INDEX=event.id_factor, FUN=sum)*(tapply(socialRate, INDEX=event.id_factor, FUN=sum)==0)+tapply(socialRate, INDEX=event.id_factor, FUN=sum)
